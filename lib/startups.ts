import { Application, ProgramTrack, readApplications } from "@/lib/applications";
...
export function startupFromApplication(application: Application) {
  ...
  return {
    ...
    programTrack: application.programTrack,
...
export function syncAcceptedApplicationsToStartups() {
  ...
  const applications = readApplications().filter(
    (application) => application.status === "Kabul" && application.programTrack,
  );
