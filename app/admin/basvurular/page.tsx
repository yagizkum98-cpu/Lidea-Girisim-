...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    const sync = async () => {
      ...
      try {
        ...
        const remote = result.applications.map(applicationFromApi);
        const merged = mergeApplicationsFromApi(remote, readApplications());
        if (JSON.stringify(merged) !== JSON.stringify(readApplications())) {
          writeApplications(merged);
        }
        setApplications(merged);
      } catch {
        // Keep the locally cached workflow available if the database is temporarily unreachable.
      }
    };
    sync();
  ...
  function changeBulkStatus(status: ApplicationStatus) {
    if (status === "Kabul") {
      window.alert("Kabul işlemi için başvuru detayından Ön Kuluçka veya Kuluçka seçmelisiniz.");
      return;
    }
