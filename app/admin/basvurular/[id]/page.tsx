...
export default function ApplicationDetailPage() {
  ...
  async function updateServer(data: { status?: string; programTrack?: ProgramTrack }) {
  ...
  async function changeStatus(nextStatus: ApplicationStatus) {
    ...
    if (nextStatus === "Kabul" && !application.programTrack) {
      setNotice("Kabul etmeden önce Ön Kuluçka veya Kuluçka programını seçin.");
      return;
    }
    ...
    const apiStatuses: Record<ApplicationStatus, string> = {
      Yeni: "NEW",
      İnceleniyor: "UNDER_REVIEW",
      "Eksik Bilgi": "MISSING_INFO",
      "Jüriye Gönderildi": "SENT_TO_JURY",
      Kabul: "ACCEPTED",
      Yedek: "WAITLIST",
      Reddedildi: "REJECTED",
    };
    ...
    try {
      await updateServer({
        status: apiStatuses[nextStatus],
        ...(nextStatus === "Kabul" ? { programTrack: application.programTrack as ProgramTrack } : {}),
      });
    } catch {
      setNotice("İşlem veritabanına kaydedilemedi. Lütfen tekrar deneyin.");
      return;
    }
  ...
  async function assignProgramTrack(programTrack: ProgramTrack) {
    if (!application) return;
    try {
      await updateServer({ programTrack });
    } catch {
      setNotice("Program seçimi veritabanına kaydedilemedi. Lütfen tekrar deneyin.");
      return;
    }
    const nextApplication = { ...application, programTrack };
    persist(nextApplication, `${programTrack} programı seçildi.`);

    const startups = readStartups();
    if (startups.some((startup) => startup.applicationId === application.id)) {
      writeStartups(
        startups.map((startup) =>
          startup.applicationId === application.id
            ? { ...startup, programTrack, updatedAt: new Date().toISOString() }
            : startup,
        ),
      );
    }
    addAdminActivity("Program türü belirlendi", `${application.startup}: ${programTrack}`);
  }
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_340px]">
          ...
          {tab === "Genel Bilgiler" ? (
            <div className="grid gap-4 md:grid-cols-2">
              {[
                ...
                ["Kabul Programı", application.programTrack || "Henüz seçilmedi"],
                ["KVKK Bilgilendirmesi", application.kvkkAccepted ? "Okundu" : "Kayıt yok"],
              ].map(([label, value]) => (
          ...
          {tab === "Girişim" ? (
            <div className="space-y-6">
              {[
                ...
                ["Programdan İstedikleri", application.programExpectations.join(", ")],
                ["Diğer Beklenti Açıklaması", application.programExpectationOther],
              ].map(([label, value]) => (
                <div key={label}>
        ...
        <aside className="space-y-6">
          <section className="rounded-lg border border-slate-200 bg-white p-5">
            <h2 className="text-lg font-black">Kabul Programı</h2>
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Başvuruyu kabul etmeden önce girişimin katılacağı programı seçin.
            </p>
            <div className="mt-4 grid grid-cols-2 gap-2">
              {(["Ön Kuluçka", "Kuluçka"] as ProgramTrack[]).map((programTrack) => (
                <button
                  type="button"
                  key={programTrack}
                  onClick={() => assignProgramTrack(programTrack)}
                  aria-pressed={application.programTrack === programTrack}
                  className={`min-h-12 rounded-md border px-3 text-sm font-black ${
                    application.programTrack === programTrack
