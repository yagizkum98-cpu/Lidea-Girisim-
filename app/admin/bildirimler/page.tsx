...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    const sync = async () => {
      ...
      try {
        ...
        const result = (await response.json()) as { notifications: Record<string, unknown>[] };
        ...
        setNotifications(
          result.notifications.map((item) =>
            normalizeNotification({
              ...
              status: statusMap[String(item.status)] || "Taslak",
              recipients: Array.isArray(item.recipients)
                ? item.recipients.map((recipient) => {
                    const value = recipient as Record<string, unknown>;
                    return {
                      name: String(value.email || ""),
                      email: String(value.email || ""),
                      role: String(value.role || "Girişimci"),
  ...
  async function submitNotification(event: FormEvent<HTMLFormElement>) {
    ...
    const typeMap: Record<NotificationType, string> = {
      Duyuru: "ANNOUNCEMENT",
      Hatırlatma: "REMINDER",
      "Durum Güncellemesi": "STATUS_UPDATE",
      "Belge / Görev": "DOCUMENT_TASK",
      Etkinlik: "EVENT",
    };
    ...
    try {
      ...
      if (!response.ok) {
        setNotice("Bildirim veritabanına kaydedilemedi. Lütfen tekrar deneyin.");
        return;
      }
      const result = (await response.json()) as { notification: { id: string } };
      notification.id = result.notification.id;
    } catch {
      setNotice("Bildirim servisine ulaşılamadı. Lütfen tekrar deneyin.");
      return;
    }
